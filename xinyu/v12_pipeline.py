# ================================================================
# v12: v7 + real anomaly supervision (minimum-change ablation)
#
# Only TWO changes vs v7:
#   1. SegHead training data: 30% good / 40% real anomaly / 30% cut-paste
#      (v7 was 50% good / 50% cut-paste, real anomalies were ONLY used
#      as cut-paste source material, never directly as training signal)
#   2. GT masks are explicitly resized to 224x224 with nearest-neighbor
#
# Everything else (DINOv2-S, multi-layer SegHead, memory bank, ensemble
# weights 0.45/0.55, train-good normalize) is identical to v7.
#
# Goal: clean attribution of LB delta to "real anomaly supervision".
# ================================================================
...
# v12: training data composition
P_GOOD = 0.30
P_REAL = 0.40
P_SYNTH = 0.30  # = 1 - P_GOOD - P_REAL
...
# ---------------- v12 SegHead training dataset ----------------
def resize_mask_to_224(mask):
    """Mask-aware nearest-neighbor resize to 224x224 (binary input -> binary output)."""
...
class TrainSynthDatasetV12(Dataset):
    """
    Per-sample, the training image is one of three types:
        - p_good probability: a normal good image with empty mask
        - p_real probability: a real anomaly image with its true GT mask
        - 1-p_good-p_real probability: cut-paste synthetic anomaly
    """
    ...
    def __getitem__(self, i):
        ...
        if r < self.p_good or len(self.sources) == 0:
            ...
            # Type B (NEW in v12): real anomaly + real GT mask
...
# ---------------- SegHead architecture (multi-layer, same as v7) ----------------
...
# ---------------- Train SegHead per class (v12) ----------------
print(f"\n=== [4/6] Train v12 SegHeads (data: {P_GOOD:.0%} good / {P_REAL:.0%} real / {P_SYNTH:.0%} synth) ===")
SEG_HEADS = {}
for class_name in sorted(ANOMALY_SOURCES.keys()):
    ...
    loader = DataLoader(TrainSynthDatasetV12(good_paths, sources, SAMPLES_PER_EPOCH, preprocess),
                        batch_size=BATCH_SIZE, num_workers=2, pin_memory=True)
    ...
    torch.save(head.state_dict(), f'/kaggle/working/seghead_v12_{class_name}.pt')
...
# ---------------- Encode + submit (same logic as v7) ----------------
...
print(f"  csv {len(df)} rows, zip {os.path.getsize('/kaggle/working/submission.zip') / 1024 / 1024:.2f} MB")
...
competitions
submit - c
adl - 2025 - 2026 - anomaly - detection \
- f / kaggle / working / submission.zip \
- m
"Day 2 v12: v7 + real anomaly supervision (30/40/30 good/real/synth)"
